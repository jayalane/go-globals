...
func writeStringToFile(fn string, content string) error { //nolint:cyclop
	...
	err = os.MkdirAll(filePath, 0o770)
	if err != nil {
	...
	cmd := exec.Command("/bin/cat", filePath+fn) //nolint:gosec
	...
	cmd = exec.Command("/bin/ls", filePath+fn) //nolint:gosec
